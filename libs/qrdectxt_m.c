...
//a substitution for iconv which simply copies bytes
size_t noconv(char** inbuf, size_t *inbytesleft, char** outbuf, size_t *outbytesleft){
    size_t counter = 0;

    //the function differs from iconv in the NULL pointer processing!!!
    ...
    if(*inbytesleft > *outbytesleft){
        counter = *outbytesleft;
    }else{
        counter = *inbytesleft;
    }
    memcpy(*outbuf, *inbuf, counter);
    *inbuf += counter;
    *outbuf += counter;
    *inbytesleft -= counter;
    *outbytesleft -= counter;

    if(*inbytesleft > *outbytesleft){
        //printf("nonconv inbyteleft > outbytesleft!\n");
        return (size_t) - 1;
    }
    //printf("noconv converted%d:\n", counter);
    return counter;
}
      ...
      for(k=0;k<qrdataj->nentries&&!err;k++){
        ...
        switch(entry->mode){
          ...
          case QR_MODE_BYTE:{
            ...
            #else
            /*size_t ileft = inleft;
            size_t oleft = outleft;*/
            err = (noconv(&in,&inleft,&out,&outleft)==((size_t)-1));//returns err if
            if(!err)sa_ntext=out-sa_text;
            /*else {
                printf("qr_code_data_list_extract_text() error acquired!\n");
                printf("inleft = %d, outleft = %d\n", ileft, oleft);
            }*/
            #endif
          }break;
        ...
        /*if(err){
            printf("In mode %d an error acquired inside  qr_code_data_list_extract_text()!\n", (uint32_t)(entry->mode));
        }*/
      }
